...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "flixd",
		Short:             "FLIX blockchain Daemon (server)",
	...
	// CLI command to print the flixd version
	rootCmd.AddCommand(version.Cmd())
	// CLI command to start and run the flixd (server)
	server.AddCommands(ctx, cdc, rootCmd, newApp, exportAppStateAndTMValidators)

	// prepare and add flags
	executor := cli.PrepareBaseCmd(rootCmd, "FLIX", app.DefaultNodeHome)
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		gApp := app.NewFlixApp(logger, db, traceStore, false, uint(1))
	...
	gApp := app.NewFlixApp(logger, db, traceStore, true, uint(1))
	return gApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
